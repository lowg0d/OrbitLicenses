...
plugin = lightbulb.Plugin("regenlicense_cmd")
...
@lightbulb.command('regenlicense', "Regenerate a existing license.")
@lightbulb.implements(lightbulb.SlashCommand)
async def regenlicense_cmd(ctx: lightbulb.Context) -> None:

    # get the user from the options & get the user's id
    user = ctx.options.user
    user_id = user.id
    id_ = sql_mg.sql_get_license_id(user_id)
    ...
    if license_mg.check_if_user_on_db(user_id) == True:
        ...
        msg = hikari.Embed(
            title=f"Are you sure you want to regen [License#{id_}]",
                ...
                async for event in stream:
                    ...
                    if event.interaction.custom_id == "yes":
                        ...
                        skipped_embed = hikari.Embed(
                            title=f"[Licence#{id_}] regenerate skipped Sucesfully",
                            color=cc.correct_color)
...
########################################################################
@regenlicense_cmd.set_error_handler
